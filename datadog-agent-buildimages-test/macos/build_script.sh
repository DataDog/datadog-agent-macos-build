...
# FIXME: Remove this once we fix the way we cache the builder setup
# in datadog-agent-macos-build, we have non-critical pip errors that
# make the script fail with set -e.
set +e
